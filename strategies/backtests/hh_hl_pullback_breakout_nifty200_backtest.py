"""
HH-HL Pullback Breakout (Flag Continuation) -- Nifty 200 Backtest (Run 2)
(2026-08-03, Karan-originated concept, follow-on to Run 1's Nifty 100 result)
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hh_hl_pullback_breakout_nifty200")
...
trades_df.to_csv(os.path.join(OUT_DIR, "hh_hl_pullback_breakout_nifty200_trades.csv"), index=False)
...
print(f"\n=== HH-HL Pullback Breakout -- Nifty 200 (signal-level, ~{NOTIONAL_PER_TRADE:,} notional/trade, CNC charges modelled) ===")
...
print(f"\nTrade log -> {OUT_DIR}/hh_hl_pullback_breakout_nifty200_trades.csv")
