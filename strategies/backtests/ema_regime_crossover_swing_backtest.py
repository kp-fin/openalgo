"""
EMA Regime Crossover — SWING variant (RS-Ratio/RS-Momentum regime filter)
20-name universe (top 10 Nifty 50 + top 10 Nifty Next 50, same static
current-constituent list as the intraday EMA Regime Crossover backtest,
see ema_regime_crossover_backtest.py and equities-system/strategies/
ema_regime_crossover.md) | 2021-07-01 -> 2026-06-30

A SEPARATE strategy from the intraday EMA Regime Crossover -- explored per
Karan's request 2026-07-19, not a modification of the live strategy. Same
entry/exit mechanics (EMA9/EMA20 crossover, 1.36xATR stop / 3xATR target)
but three confirmed differences:

  1. Entirely daily-bar based -- no 15m/30m intraday data at all. This is
     the swing analog: holding periods of days to weeks rather than
     intraday-only.
  2. Regime filter is JdK RRG-style RS-Ratio/RS-Momentum (stock vs its
     respective index -- NIFTY for the Nifty 50 names, NIFTYNXT50 for the
     Nifty Next 50 names) INSTEAD OF EMA(200) -- a full replacement, not an
     additional gate alongside EMA200 (confirmed with Karan).
  3. No hard/time-based exit -- a position rides until stop, target, or an
     opposite EMA9/20 crossover closes it. Trades still open at the
     backtest's end date are closed at the last available price, labeled
     END_OF_DATA, rather than silently dropped.

Regime definition (JdK RRG, 10-bar z-score window + 2-bar smoothing --
published JdK defaults, named here so they're easy to change):
  RS          = 100 * (stock_close / index_close)
  RS-Ratio    = smooth(100 + (RS - SMA(RS,10)) / STDEV(RS,10), 2)
  RS-Momentum = smooth(100 + (RS-Ratio - SMA(RS-Ratio,10)) / STDEV(RS-Ratio,10), 2)

  BULL regime: RS-Ratio > 100 (outperforming) AND
               (RS-Momentum > 100 OR RS-Momentum risen over the last
               RS_MOMENTUM_SLOPE_LOOKBACK days) -- "accelerating, or moving
               towards acceleration."
  BEAR regime: RS-Ratio < 100 (underperforming) AND
               (RS-Momentum < 100 OR RS-Momentum fallen over the last
               RS_MOMENTUM_SLOPE_LOOKBACK days) -- mirrored, confirmed with
               Karan 2026-07-19.

New entries only blocked outside the regime's allowed direction; an already
open position rides its own exit through a regime flip -- same convention
as the intraday version.

Sizing: fixed 12.5% of TOTAL_CAPITAL per trade (flat, unchanged from the
intraday version -- not asked to be different). Max 6 concurrent positions
across the whole 20-name universe; skip a new signal beyond that cap.

Historical-accuracy caveat (same as the intraday backtest): today's
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_swing")
...
for symbol in UNIVERSE:
    ...
    rs_momentum_falling = df["rs_momentum"] < df["rs_momentum"].shift(RS_MOMENTUM_SLOPE_LOOKBACK)

    bull_regime = (df["rs_ratio"] > 100) & ((df["rs_momentum"] > 100) | rs_momentum_rising)
    bear_regime = (df["rs_ratio"] < 100) & ((df["rs_momentum"] < 100) | rs_momentum_falling)
    df["regime"] = np.where(bull_regime, "BULL", np.where(bear_regime, "BEAR", "NEUTRAL"))
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_swing_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover SWING Backtest ===")
