"""
HIGH-BETA UNIVERSE VARIANT (2026-07-30): identical test #8 config (narrow
window 11:45-12:45, SHORT-only, 2.2xATR stop / 3.0xATR target, REVERSE_CROSS,
15:00 hard exit, same charges model) except restricted to the top 15 names by
trailing realized daily-return volatility (2021-07-01 -> 2026-06-30, annualized
stdev of daily log returns, computed causally -- see
ema_regime_crossover_high_beta_rank.py) out of the same 59-name universe.
Isolates the universe-size/volatility variable cleanly against the known
least-bad baseline, independent of the v2 entry-logic experiments.

EMA Regime Crossover -- Narrow Window (11:45-12:45) + Wide Stop + SHORT-only
...
Two levers have each independently improved on the combined midday+wide-stop
result (WR(net) 42.4%, Sharpe(net) -1.23, maxDD 64.5%):
  - Narrowing the window further to 11:45-12:45 (both directions):
        WR(net) 44.4%, Sharpe(net) -0.73, maxDD 34.0%
  - Restricting to SHORT-only on the midday (11:30-13:30) window:
        WR(net) 43.8%, Sharpe(net) -0.58 (best yet), maxDD 28.9% (best yet)

Both levers work by cutting the LONG side's drag (LONG PF(net) 0.82 vs
SHORT PF(net) 1.00 in the narrow-window test) and/or trade count (less
cost/variance drag). This variant combines all three: narrow window
(11:45-12:45) + wide stop (2.2xATR) + SHORT-only, to test whether they
compound favourably like window+stop did previously.

Universe expanded per Karan's request (2026-07-30) from the prior 20-name
set (top 10 Nifty 50 + top 10 Nifty Next 50) to the full Nifty 50 (49
names, same pinned snapshot used by the live strategy's monthly universe
refresh in ema_regime_crossover_signal.py) + top 10 Nifty Next 50 by
liquidity (unchanged) = 59 names. Everything else (regime, entry signal,
sizing, both circuit-breaker mechanics, charges model) is identical to the
...
FULL_UNIVERSE = NIFTY_50_FULL + NEXT50_TOP10

# Top 15 by trailing realized daily-return volatility (annualized stdev of
# daily log returns, 2021-07-01 -> 2026-06-30), from
# ema_regime_crossover_high_beta_rank.py / ema_regime_crossover_high_beta_rank/high_beta_ranking.csv
HIGH_BETA_15 = ["ADANIGREEN", "ADANIENSOL", "ADANIENT", "ADANIPOWER", "ETERNAL",
                "LODHA", "TMCV", "VEDL", "ADANIPORTS", "CGPOWER", "MAXHEALTH",
                "CANBK", "SHRIRAMFIN", "TRENT", "JIOFIN"]
UNIVERSE = HIGH_BETA_15

OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_test8_high_beta")
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_wide_stop_short_only_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Narrow Window (11:45-12:45) + Wide Stop (2.2xATR), SHORT-only ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_wide_stop_short_only_trades.csv")
