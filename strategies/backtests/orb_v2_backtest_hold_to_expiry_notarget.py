"""
ORB_Spread — Hold to Expiry Variant, STOP-ONLY / no target (exploratory)
Nifty 50 Spot | 2021-07-01 -> 2026-06-30

Follow-up to orb_v2_backtest_hold_to_expiry.py, which found that keeping the
+40pt target active meant hold-to-expiry barely differed from the same-day
15:15 exit (only 1 of 794 trades ever reached expiry without hitting
target/stop first). This variant tests the other option Karan flagged but
didn't pick initially: drop the +40pt target entirely, keep only the -25pt
stop as a risk cap, and let winners run all the way to expiry uncapped.
...
Exit logic changes from the current live design:
  - -25pt stop only, checked on 5m closes, no target -- the walk-forward
    continues day-by-day (using each day's 09:15-15:30 5m bars) until that
    week's expiry, capped only by the stop or expiry settlement.
  ...
  - If the stop isn't hit by expiry, the trade settles at the expiry day's
    last available 5m close before 15:30 (EXPIRY_SETTLE) -- spot-points
    intrinsic value, same convention as every other ORB_Spread backtest (no
    historical option chain/IV data exists for this period).
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "orb_v2_hold_to_expiry_notarget")
...
print("\n" + "=" * 90)
print("HOLD TO EXPIRY, STOP-ONLY — spot points (naked proxy)")
print("=" * 90)
res_spot = summarise(trades, "pnl_pts", "Hold to expiry (naked spot proxy)")
...
print("\n" + "=" * 90)
print(f"HOLD TO EXPIRY, STOP-ONLY — {SPREAD_WIDTH}pt/{SPREAD_COST_PCT*100:.0f}% spread overlay (same convention as adopted same-day model)")
print("=" * 90)
res_spread = summarise(trades, "spread_pnl", f"Hold to expiry ({SPREAD_WIDTH}pt/{SPREAD_COST_PCT*100:.0f}% spread)")

print("\n" + "=" * 90)
print("COMPARISON — current live (same-day 15:15 hard exit) vs hold-to-expiry variant")
print("=" * 90)
...
if os.path.exists(baseline_csv):
    ...
    b_spot = summarise(baseline, "pnl_pts", "Same-day 15:15 hard exit (spot)") or {}
...
print("\nBacktest complete. Output: orb_v2_hold_to_expiry/hold_to_expiry_trades.csv")
