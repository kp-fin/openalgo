"""
Gap-and-Go -- Walk-Forward Split Check (2026-08-03)

Splits the already-generated 5-year trade log (gap_and_go_trades.csv, portfolio-
simulated with the same concurrency cap / daily circuit breaker as the full run)
at the window midpoint and recomputes WR/PF/Sharpe/maxDD independently for each
half, the same check that caught HH-HL Pullback Breakout's inflated aggregate
Sharpe (9.93 aggregate vs. a much more modest 5.40 second-half). Trades aren't
re-simulated -- the portfolio-level constraints were already applied causally in
chronological order during the original run; splitting the accepted-trade list
post-hoc for reporting is the same method HH-HL's own walk-forward check used.
"""
...
IN_CSV = os.path.join(os.path.dirname(os.path.abspath(__file__)), "gap_and_go", "gap_and_go_trades.csv")
