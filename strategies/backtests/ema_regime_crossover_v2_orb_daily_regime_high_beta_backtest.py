"""
HIGH-BETA UNIVERSE VARIANT (2026-07-30): identical to Concept B below except the
universe is restricted to the top 15 names, by trailing realized daily-return
volatility (2021-07-01 -> 2026-06-30, annualized stdev of daily log returns,
computed causally from price history only -- see
ema_regime_crossover_high_beta_rank.py), out of the test #8 59-name universe.
Hypothesis (Karan): higher average daily range per name -> larger average win
size relative to the largely-fixed per-trade charges, without needing an even
wider R:R target that then can't complete before the 15:00 hard exit.
...
FULL_UNIVERSE = sorted(set(NIFTY50 + NEXT50_TOP10))
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_v2_orb_daily_regime_high_beta")
