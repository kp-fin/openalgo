"""
EMA Regime Crossover -- Midday Window + SHORT-only + RSI Strict Early Loss-Cut (threshold=60)
...
Test #15 tried this same widened-window + RSI-loss-cut combination with
RSI_LOSS_CUT_SHORT=50 (the neutral midline) -- DECISIVELY REJECTED, maxDD
93.9%. Root cause: RSI(14) on 15-minute bars oscillates through 50
constantly as ordinary short-term noise, so the threshold fired on 56.1%
of all trades (n=2361) with a 1.1% win rate -- it was cutting almost every
trade within a few bars of entry regardless of whether it was actually
deteriorating.

This variant (test #16) keeps everything else from test #15 identical and
only tightens RSI_LOSS_CUT_SHORT from 50 to 60 -- meaningfully further
from the noisy midline, intended to fire only on a genuine, sustained
momentum reversal rather than routine oscillation.
...
SHORT_ONLY = True
RSI_PERIOD = 14
RSI_LOSS_CUT_SHORT = 60  # SHORT: exit once RSI recovers meaningfully past neutral (momentum faded)
RSI_LOSS_CUT_LONG = 40   # LONG symmetric threshold; unused under SHORT_ONLY
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_midday_short_only_rsi_loss_cut_60")
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_midday_short_only_rsi_loss_cut_60_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_midday_short_only_rsi_loss_cut_60_trades.csv")
