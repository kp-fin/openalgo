"""
EMA Regime Crossover -- Narrow Window + SHORT-only + RSI Oversold Exit Confirmation
...
Test #8 (narrow window 11:45-12:45 + fixed wide stop 2.2xATR + TARGET
3.0xATR + REVERSE_CROSS + SHORT-only + 59-name universe) remains the best
result: WR(net) 44.7%, Sharpe(net) -0.24, PF(net) 0.96, maxDD 29.8%. Five
exit-logic variants in a row (tests #9-#13) that removed or loosened an
existing exit signal all underperformed it. Karan explicitly confirmed
test #8 is fine as the base and asked to ADD a new exit trigger on top of
it rather than remove one: an RSI(14) oversold confirmation for SHORT
exits (RSI <= 25 closes the position at that bar's close), on the
reasoning that an oversold reading on a SHORT position signals the
down-move is exhausted and a bounce/reversal is more likely -- i.e. a
different kind of early profit-lock than REVERSE_CROSS's EMA-based signal.

RSI_EXIT is checked after STOP and TARGET (same worst-case-first ordering
every variant in this series uses) but before REVERSE_CROSS, since it's a
level check on a smoothed oscillator that can fire independently of and
potentially earlier than an EMA crossover. Standard RSI(14) with Wilder
smoothing (same ewm(alpha=1/period) style as the existing ATR calc, no new
parameter invented). LONG-side symmetry (RSI >= 75 overbought exit) is
included for correctness but SHORT_ONLY=True means it never fires in this
run.

Everything else (regime, entry signal, window, fixed stop/target
multiples, REVERSE_CROSS, HARD_EXIT, SHORT-only, 59-name universe, sizing,
both circuit-breaker mechanics, charges model) is identical to test #8 --
only the new RSI_EXIT trigger is added.
"""
...
SHORT_ONLY = True
RSI_PERIOD = 14
RSI_EXIT_OVERSOLD = 25    # SHORT exit trigger
RSI_EXIT_OVERBOUGHT = 75  # LONG exit trigger (symmetric; unused under SHORT_ONLY)
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_narrow_window_short_only_rsi_exit")
...
print(f"Universe: {len(UNIVERSE)} names | Entry window: {ENTRY_WINDOW_START} - {ENTRY_WINDOW_END} IST only | "
      f"SHORT_ONLY={SHORT_ONLY} | RSI_EXIT_OVERSOLD={RSI_EXIT_OVERSOLD}")
all_trades = []
skipped = {}
rejected_window = 0
        ...
        for ts2, bar2 in rest.iterrows():
            ...
            if direction == "LONG":
                ...
                if bar2["high"] >= target_px:
                    ...
                    break
                if bar2["rsi"] >= RSI_EXIT_OVERBOUGHT:
                    exit_px, exit_ts, reason = bar2["close"], ts2, "RSI_EXIT"
                    break
                if bar2["bear_cross"]:
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_short_only_rsi_exit_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Narrow Window + SHORT-only + RSI Oversold Exit ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_short_only_rsi_exit_trades.csv")
