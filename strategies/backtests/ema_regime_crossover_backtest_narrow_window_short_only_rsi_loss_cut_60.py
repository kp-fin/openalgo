"""
EMA Regime Crossover -- Narrow Window (test #8) + SHORT-only + RSI(60) Strict Early Loss-Cut
...
result: WR(net) 44.7%, Sharpe(net) -0.24, PF(net) 0.96, maxDD 29.8%.

Test #14 added RSI<=25 (oversold, a profit-lock role) on top of test #8 --
too rare to matter (8/2441 trades, 0.3%). Test #15 flipped RSI to a
loss-cut role (RSI>=50) on a WIDENED window -- decisively rejected, maxDD
93.9%, because RSI(14) at 15-min bars oscillates through 50 constantly and
fired on 56.1% of trades. Test #16 kept the widened window but tightened
the threshold to RSI>=60.

This variant (test #17) tests the other half of Karan's 2026-07-30
request: keep test #8's narrow window (11:45-12:45) UNCHANGED and layer
the same RSI(60) loss-cut role on top of it instead, rather than combining
it with the wider window. RSI_LOSS_CUT_SHORT=60 is checked after STOP but
BEFORE TARGET (loss-cut role -- catch deterioration before it develops,
same ordering as tests #15/#16), then REVERSE_CROSS, then HARD_EXIT.
...
only the new RSI_LOSS_CUT trigger is added.
"""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_narrow_window_short_only_rsi_loss_cut_60")
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_short_only_rsi_loss_cut_60_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Narrow Window + SHORT-only + RSI Strict Early Loss-Cut ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_short_only_rsi_loss_cut_60_trades.csv")
