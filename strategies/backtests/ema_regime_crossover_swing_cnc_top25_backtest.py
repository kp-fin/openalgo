"""
EMA Regime Crossover -- Swing CNC, Middle-Ground Universe Size Test (Top-25
...
just a LARGER top-N slice (top 25) instead of top-15. Ranking CSV:
`ema_regime_crossover_high_beta_rank_nifty100/high_beta_ranking_nifty100.csv`.

TOP-25 UNIVERSE (by descending ann_vol_pct):
...
PRIMARY test, apples-to-apples vs. Run 3, per Karan's brief. The top-25 list
contains 5 Adani-group names (ADANIGREEN, ADANIENSOL, ADANIENT, ADANIPOWER,
ADANIPORTS -- unlike Run 3's list this is genuinely NOT hardcoded, it's just
where the volatility ranking lands), 2 Vedanta-group (VEDL, HINDZINC), and 2
...
# ---- Universe: top-25 by ann. realized vol, ranked from the FULL Nifty 100 pool ----
...
GROUP_OF = {
    ...
    "ADANIENSOL": "ADANI_GROUP", "ADANIGREEN": "ADANI_GROUP",
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "ema_regime_crossover_swing_cnc_top25")
...
print(f"Universe: {len(UNIVERSE)} names (top-25 by ann. realized vol, ranked from full Nifty 100 pool) | "
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_swing_cnc_top25_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Swing CNC, Top-25 (from Nifty 100 pool) Universe "
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_swing_cnc_top25_trades.csv")
