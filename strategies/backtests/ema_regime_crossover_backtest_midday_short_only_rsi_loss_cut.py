"""
EMA Regime Crossover -- Midday Window + SHORT-only + RSI Strict Early Loss-Cut
...
Test #8 (narrow window 11:45-12:45 + fixed stop 2.2xATR + TARGET 3.0xATR +
REVERSE_CROSS + SHORT-only + 59-name universe) remains the best result:
WR(net) 44.7%, Sharpe(net) -0.24, PF(net) 0.96, maxDD 29.8%. Test #14
(RSI<=25 oversold exit, added on top of test #8 as a profit-lock signal)
only fired on 8/2441 trades (0.3%) -- too rare within the 1-hour narrow
window to move anything meaningfully.

Karan asked (2026-07-30) for a different combination: widen the entry
window back out (more opportunities) while using RSI as a "strict
observer" to cut a trade EARLY, before it bleeds toward the fixed stop --
i.e. RSI's role flips from a rare profit-lock (oversold, RSI<=25) to a
frequent early-loss-cut (RSI recovering back toward/through neutral, which
for a SHORT position means the bearish momentum that justified entry has
already faded). RSI_LOSS_CUT = RSI >= 50 (neutral line) is the strictest
defensible reading of "cut as soon as the edge is gone" without being
hair-trigger on normal noise; symmetric LONG threshold (RSI <= 50) is
included for correctness but SHORT_ONLY=True means it never fires here.

Window widened from the narrow 11:45-12:45 band back to the midday
11:30-13:30 band (previously tested and validated as the second-best
window on 2026-07-29, before the narrowing campaign that produced test #8)
-- doubling opportunity count versus test #8, on the reasoning that the
new RSI loss-cut should let a wider window's extra (necessarily lower
average quality) trades be culled early instead of running to the full
2.2xATR stop.

Order of checks within a bar (worst-case-first, consistent with every
other variant in this series): STOP, then RSI_LOSS_CUT (the new early
risk check), then TARGET, then REVERSE_CROSS, then HARD_EXIT-by-time.

Everything else (regime, entry signal, fixed stop/target multiples,
REVERSE_CROSS, HARD_EXIT, SHORT-only, 59-name universe, sizing, both
circuit-breaker mechanics, charges model) is identical to test #8.
"""
...
SHORT_ONLY = True
RSI_PERIOD = 14
RSI_LOSS_CUT_SHORT = 50  # SHORT: exit once RSI recovers back through neutral (momentum faded)
RSI_LOSS_CUT_LONG = 50   # LONG symmetric threshold; unused under SHORT_ONLY
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_midday_short_only_rsi_loss_cut")
...
print(f"Universe: {len(UNIVERSE)} names | Entry window: {ENTRY_WINDOW_START} - {ENTRY_WINDOW_END} IST only | "
      f"SHORT_ONLY={SHORT_ONLY} | RSI_LOSS_CUT_SHORT={RSI_LOSS_CUT_SHORT}")
all_trades = []
skipped = {}
rejected_window = 0
...
for symbol in UNIVERSE:
    ...
    df15 = df15.dropna(subset=["atr", "regime", "rsi"])
        ...
        for ts2, bar2 in rest.iterrows():
            ...
            if direction == "LONG":
                if bar2["low"] <= stop_px:
                    ...
                    break
                if bar2["rsi"] <= RSI_LOSS_CUT_LONG:
                    exit_px, exit_ts, reason = bar2["close"], ts2, "RSI_LOSS_CUT"
                    break
                ...
                if bar2["high"] >= stop_px:
                    exit_px, exit_ts, reason = stop_px, ts2, "STOP"
                    break
                if bar2["rsi"] >= RSI_LOSS_CUT_SHORT:
                    exit_px, exit_ts, reason = bar2["close"], ts2, "RSI_LOSS_CUT"
                    break
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_midday_short_only_rsi_loss_cut_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_midday_short_only_rsi_loss_cut_trades.csv")
