"""
HH-HL Pullback Breakout -- Nifty 200, PORTFOLIO MC=6 + dry-up + BAND-STAY/CEILING
(2026-08-11 variant of hh_hl_pullback_breakout_nifty200_portfolio_backtest.py)

SPEC baseline: equities-system/strategies/hh_hl_pullback_breakout.md.
THIS FILE is a NEW variant -- does not overwrite the deployed baseline.

Same as baseline MC=6 + DRY_VOLUME_MULT=0.8, plus two invalidations after
first close inside the 40-60% retracement band:
  - BAND-STAY: after band entry, check breakout first; if not a breakout and
    close leaves [retrace_lo, retrace_hi], invalidate the impulse.
  - CEILING: if cons_days reaches MAX_CONSOLIDATION_DAYS (20) with no valid
    breakout, invalidate. Range/vol updates only on days that stay in-band.

Baseline (no band-stay/ceiling) reference: 28 trades, PF(net) 5.09,
Sharpe(net) 9.93 on 2021-07-01->2026-06-30. Do not treat this variant as
live-deployable until results are reviewed and explicitly accepted.

PORTFOLIO PARAMETERS -- identical to the Nifty 100 portfolio run
(hh_hl_pullback_breakout_nifty100_portfolio_backtest.py), reused byte-for-byte
from EMA Swing-59/100's proven config: ALLOCATED_CAPITAL=Rs 2,50,000,
MAX_CONCURRENT=6, MAX_PER_GROUP=2, capital_per_trade = ALLOCATED_CAPITAL /
MAX_CONCURRENT, ASSUMED_LEVERAGE=1 (CNC, no leverage). See the Nifty 100
portfolio script's docstring for the CONCURRENCY-SLOT SIMPLIFICATION note
...
- "Higher high / higher low" check only compares the impulse leg's swingL/
  ...
  silently dropped.
- Consolidation window: running high/low from first in-band close, updated
  ONLY on days that remain inside [retrace_lo, retrace_hi]. Invalidations:
  close < px_L (impulse swing low), close leaves the band without a valid
  breakout, or cons_days hits MAX_CONSOLIDATION_DAYS without breakout.
  Breakout is always checked before band-leave / ceiling checks so a
  volume-confirmed close above cons_high can fire even if that close is
  above retrace_hi.
...
MAX_CONSOLIDATION_DAYS = 20          # band-stay ceiling -- invalidate if no breakout by then
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hh_hl_pullback_breakout_nifty200_portfolio_bandstay")
...
print(f"Universe: {len(UNIVERSE)} names (Nifty 200) | ZigZag={ZIGZAG_PCT*100:.0f}% | "
      f"retrace band={RETRACE_LO*100:.0f}-{RETRACE_HI*100:.0f}% | "
      f"cons={MIN_CONSOLIDATION_DAYS}-{MAX_CONSOLIDATION_DAYS}d | dry_up={DRY_VOLUME_MULT}x | "
      f"vol_mult={VOLUME_MULT}x | leg1_RR=1:{LEG1_RR:.0f} | trail={TRAIL_ATR_MULT}xATR | BAND-STAY")
...
for symbol in UNIVERSE:
    ...
    for cross_i in cross_days:
        ...
        # 4) band entry -> cons in-band only (5..20d) -> volume breakout.
        # Invalidate: close < px_L, leave band without breakout, or hit ceiling.
        band_entry_i = None
        ...
        for i in range(confirm_H, n):
            ...
            # already in consolidation tracking -- breakout before band-leave
            ...
            if close > retrace_hi_px or close < retrace_lo_px:
                break  # left 40-60% band without a valid breakout
            if cons_days >= MAX_CONSOLIDATION_DAYS:
                break  # ceiling -- no breakout within MAX_CONSOLIDATION_DAYS
...
trades_df.to_csv(os.path.join(OUT_DIR, "hh_hl_pullback_breakout_nifty200_portfolio_bandstay_trades.csv"), index=False)
...
print(f"\n=== HH-HL BAND-STAY/CEILING -- Nifty 200, MC={MAX_CONCURRENT}, dry-up={DRY_VOLUME_MULT}, "
      f"cons_max={MAX_CONSOLIDATION_DAYS}d (Rs{ALLOCATED_CAPITAL:,}, max {MAX_PER_GROUP}/group) ===")
...
print(f"\nTrade log -> {OUT_DIR}/hh_hl_pullback_breakout_nifty200_portfolio_bandstay_trades.csv")
