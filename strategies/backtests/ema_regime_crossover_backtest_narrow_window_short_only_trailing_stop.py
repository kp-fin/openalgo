"""
EMA Regime Crossover -- Narrow Window + SHORT-only + Trailing Stop (armed once favourable)
...
-0.24, PF(net) 0.96, maxDD 29.8%. Tests #9/#10 (removing / conditionally
skipping REVERSE_CROSS) closed out that lever -- REVERSE_CROSS structurally
never fires while a trade is favourable, so there was nothing to soften.

This variant tries a genuine trailing STOP instead: before the trade is
favourable, the original fixed stop (2.2xATR) applies exactly as in test
#8. Once favourable excursion from entry reaches TRAIL_ARM_MULT x ATR, the
fixed stop is replaced by a trailing level TRAIL_ATR_MULT x ATR behind the
running favourable extreme (lowest low since entry, for SHORT), recomputed
every bar. With ARM_MULT == TRAIL_MULT == 2.0, the trailing level starts
exactly at breakeven (entry price) the instant it arms and only tightens
further as the trade extends -- it is always at least as protective as the
original 2.2xATR stop from the moment it activates. These are the same
ARM=2.0/TRAIL=2.0 parameters Karan confirmed 2026-07-28 for the (separate,
gross-only, partial-exit) trailing-target ablation in
ema_regime_crossover_trailing_target_backtest.py -- reused here rather than
picked arbitrarily, though this variant is a full trailing STOP (single
exit at trail-touch), not a partial profit-take.

TARGET and REVERSE_CROSS remain as in test #8 (fixed 3.0xATR target,
REVERSE_CROSS still checked). HARD_EXIT unchanged (15:00 IST). Order of
checks within a bar: active stop (fixed pre-arm / trailing post-arm) first,
then TARGET, then REVERSE_CROSS, then HARD_EXIT-by-time -- same
worst-case-first convention as every other variant in this series.

Everything else (regime, entry signal, window, target multiple, SHORT-only,
59-name universe, sizing, both circuit-breaker mechanics, charges model) is
identical to test #8 for a clean comparison.
"""
...
SHORT_ONLY = True
TRAIL_ARM_MULT = 2.0   # favourable excursion (x ATR) needed to arm the trailing stop
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_narrow_window_short_only_trailing_stop")
...
def simulate_exit(direction, entry_price, atr, fixed_stop_px, target_px, rest_df):
    ...
    for ts2, bar2 in rest_df.iterrows():
        ...
        if direction == "LONG":
            ...
            if not armed and (running_extreme - entry_price) >= TRAIL_ARM_MULT * atr:
                armed = True
            active_stop = (running_extreme - TRAIL_ATR_MULT * atr) if armed else fixed_stop_px
            if bar2["low"] <= active_stop:
                reason = "TRAIL_STOP" if armed else "STOP"
                return active_stop, ts2, reason
            if bar2["high"] >= target_px:
                return target_px, ts2, "TARGET"
            if bar2["bear_cross"]:
                return bar2["close"], ts2, "REVERSE_CROSS"
        else:
            running_extreme = min(running_extreme, bar2["low"])
            if not armed and (entry_price - running_extreme) >= TRAIL_ARM_MULT * atr:
                armed = True
            active_stop = (running_extreme + TRAIL_ATR_MULT * atr) if armed else fixed_stop_px
            if bar2["high"] >= active_stop:
                reason = "TRAIL_STOP" if armed else "STOP"
                return active_stop, ts2, reason
            ...
            if bar2["bull_cross"]:
                return bar2["close"], ts2, "REVERSE_CROSS"
        if t >= HARD_EXIT:
            return bar2["close"], ts2, "HARD_EXIT"
    return None, None, None


print(f"Universe: {len(UNIVERSE)} names | Entry window: {ENTRY_WINDOW_START} - {ENTRY_WINDOW_END} IST only | "
      f"SHORT_ONLY={SHORT_ONLY} | TRAIL_ARM={TRAIL_ARM_MULT}xATR | TRAIL_DIST={TRAIL_ATR_MULT}xATR")
all_trades = []
skipped = {}
rejected_window = 0
...
for symbol in UNIVERSE:
    ...
    for i in range(len(df15) - 1):
        ...
        exit_px, exit_ts, reason = simulate_exit(direction, entry_price, atr, stop_px, target_px, rest)
        if exit_px is None:
            continue
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_short_only_trailing_stop_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Narrow Window + SHORT-only + Trailing Stop ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_short_only_trailing_stop_trades.csv")
