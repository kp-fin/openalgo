"""
EMA Regime Crossover -- Gap-Aligned Entry Filter Variant
...
Prompted by a question about whether ScanX's "Gap Scanner" could help.
Checked ScanX directly (2026-07-29): no dedicated gap tool is listed among
its screener categories, and even if one exists among its 1500+ community
screeners, ScanX has no public API -- everything is scraped from the live
web page (today's values only), so it has no historical gap data usable
for a 2021-2026 backtest regardless. The underlying idea is trivial to
test without ScanX, though: overnight gap % (today's open vs yesterday's
close) is directly computable from the daily OHLC already available via
OpenAlgo's history endpoint.

Hypothesis under test: require the day's opening gap to align with the
signal's direction -- LONG signals only on days the stock gapped UP,
SHORT signals only on days it gapped DOWN. Tests whether a same-direction
gap confirms genuine same-day momentum (worth trading) versus a crossover
firing against the day's initial move (more likely a mean-reversion trap
that later gets stopped out) -- an independent, third lever alongside the
wide-stop and midday-entry-window variants tested in parallel today.

Gap is computed once per (symbol, day) from daily bars: gap_pct =
(day_open - prev_day_close) / prev_day_close. A signal's calendar day gap
sign must match its direction (gap_pct > 0 for LONG, < 0 for SHORT); flat/
zero-gap days reject both directions.

Everything else (regime, entry signal, exit mechanics, universe, window,
sizing, both circuit-breaker mechanics, charges model) is identical to the
current live config's own backtest for a clean comparison.
"""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_gap_aligned")
...
print(f"Universe: {len(UNIVERSE)} names | Filter: signal direction must match the day's opening gap direction")
all_trades = []
skipped = {}
rejected_gap = 0
...
for symbol in UNIVERSE:
    ...
    # Daily gap: today's session open vs prior session's close.
    daily = df15.resample("1D").agg({"open": "first", "close": "last"}).dropna(subset=["open"])
    daily["prev_close"] = daily["close"].shift(1)
    daily["gap_pct"] = (daily["open"] - daily["prev_close"]) / daily["prev_close"]
    gap_by_date = daily["gap_pct"].to_dict()
    gap_by_date = {k.date(): v for k, v in gap_by_date.items()}
    ...
    for i in range(len(df15) - 1):
        ...
        gap_pct = gap_by_date.get(entry_ts.date())
        ...
        if direction == "LONG" and gap_pct <= 0:
            rejected_gap += 1
            continue
        if direction == "SHORT" and gap_pct >= 0:
            rejected_gap += 1
            continue
...
print(f"\nSignal candidates surviving gap-alignment filter: {len(trades_df)} (rejected: {rejected_gap})")
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_gap_aligned_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Gap-Aligned Entry Filter ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_gap_aligned_trades.csv")
