"""
ORB v2 — ATR Exit Variants Comparison — VectorBT-style Backtest
Nifty 50 Spot | 5m candles | 2021-07-01 → 2026-06-30

Follow-up to orb_v2_backtest_atr_trail_intraday.py, which validated that
intraday 5m ATR(14) (not daily ATR — see that script's docstring for why)
is the right scale for an ORB v2 trailing stop, then found pure ATR
trailing (mult=1.5, no fixed target) came back roughly a wash vs. the
fixed +40/-25 baseline (slightly worse win rate/avg P&L, ~flat PF).

This script runs FOUR variants side by side on the identical signal set:
  1. FIXED    — baseline: +40pt target / -25pt stop (unchanged reference)
  2. TRAIL_1.5x — pure ATR trailing stop, no fixed target, mult=1.5 (carried
                  over from the prior script for continuity)
  3. TRAIL_2.0x — pure ATR trailing stop, no fixed target, mult=2.0 (wider
                  stop — does giving trades more room change the picture?)
  4. HYBRID   — keeps the fixed +40pt target for profit-taking, but
                  replaces the fixed -25pt STOP with an ATR trailing stop
                  (mult=1.5) — protection that tightens as the trade moves
                  in favor, while still banking the full target if hit.

Same entry logic, same intraday-ATR computation (continuous 5m ATR with
overnight gap excluded from the first bar of each day), same no-lookahead
guarantees as the prior two scripts.

P&L in spot points (proxy), same convention throughout.
"""
...
ATR_MULT_TIGHT  = 1.5  # used for TRAIL_1.5x and the HYBRID stop
ATR_MULT_WIDE   = 2.0  # used for TRAIL_2.0x

OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "orb_v2_atr_variants")
...
gap_hi = (df["high"] - prev_close).abs().where(~is_first_bar, 0.0)
gap_lo = (df["low"]  - prev_close).abs().where(~is_first_bar, 0.0)
...
def simulate_atr_trail(entries_df, df, atr_mult, use_fixed_target):
    """Pure trailing-stop exit (use_fixed_target=False) or hybrid: fixed
    target + ATR trailing stop in place of the fixed stop (use_fixed_target=True)."""
    out = entries_df.copy()
    out["exit_time"], out["exit_price"], out["pnl_pts"], out["reason"] = pd.NaT, np.nan, np.nan, ""
    ...
    for row_i, row in out.iterrows():
        ...
        for ts, bar in day_bars[day_bars.index >= row["entry_time"]].iterrows():
            ...
            if use_fixed_target and pnl >= TARGET_PTS:
                reason = "TARGET"
            elif hit_stop:
                reason = "TRAIL_STOP"
            elif t >= HARD_EXIT:
...
variants = {
    "FIXED":      simulate_fixed(entries_df, df),
    "TRAIL_1.5x": simulate_atr_trail(entries_df, df, ATR_MULT_TIGHT, use_fixed_target=False),
    "TRAIL_2.0x": simulate_atr_trail(entries_df, df, ATR_MULT_WIDE,  use_fixed_target=False),
    "HYBRID":     simulate_atr_trail(entries_df, df, ATR_MULT_TIGHT, use_fixed_target=True),
}
...
for label, trades in variants.items():
    ...
    print(label)
    print("=" * 70)
    results[label] = summarise(trades, label)

print("\n" + "=" * 90)
print("COMPARISON — all variants, identical signal set")
print("=" * 90)
header = f"  {'Metric':<15}" + "".join(f"{k:>15}" for k in variants)
print(header)
for metric, fmt in [("n", "{:>15d}"), ("wr", "{:>15.1f}"), ("avg", "{:>15.2f}"),
                     ("total", "{:>15.1f}"), ("pf", "{:>15.2f}")]:
    row = f"  {metric:<15}"
    for k in variants:
        v = results.get(k, {}).get(metric)
        row += fmt.format(v) if v is not None else f"{'—':>15}"
    print(row)

# ── Save outputs ──────────────────────────────────────────────────────────────
for label, trades in variants.items():
    path = os.path.join(OUT_DIR, f"orb_v2_trades_{label.lower().replace('.', '')}.csv")
...
for label, trades in variants.items():
    try:
        ...
        qs.reports.html(eq, output=os.path.join(OUT_DIR, f"orb_v2_tearsheet_{label.lower().replace('.', '')}.html"),
