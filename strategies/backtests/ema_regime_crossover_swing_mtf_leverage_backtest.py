"""
EMA Regime Crossover -- Swing MTF Leverage Variant (2026-07-30)

Follow-on to the CNC swing Run 3 (v3 tightstop) backtest, checking whether real
Dhan MTF (margin trading facility) leverage improves capital efficiency and/or
Sharpe versus the no-leverage CNC baseline. Uses REAL per-symbol leverage
figures already surveyed against Dhan's live margin-calculator API
(`mtf_leverage_survey.csv`, queried 2026-07-19/20 for the Nifty 100 pool,
covers all 15 high-beta names here), not an assumed/flat multiplier. Adds a
genuine MTF interest cost: Dhan's published rate is 12.49% p.a. (`dhan.co/
margin-trading-facility/`, verified 2026-07-30), charged daily on the borrowed
portion of the position (`position_value - own_capital_per_trade`) for the
actual number of trading days held. All else (regime/entry, exit scheme,
concentration cap, CNC delivery charges) identical to Run 3 -- this isolates
the leverage/interest variable alone.
""" + """
...
# ---- Sizing (MTF: real per-symbol leverage from Dhan's live margin calculator) ----
ALLOCATED_CAPITAL = 250_000  # matches this strategy's existing paper-mode capital figure
MAX_CONCURRENT = 6
MTF_INTEREST_PCT_PA = 0.1249  # Dhan MTF rate, verified 2026-07-30 (dhan.co/margin-trading-facility/)

# Real per-symbol leverage, queried 2026-07-19/20 against Dhan's live margin
# calculator (mtf_leverage_survey.csv) -- NOT an assumed flat multiplier.
MTF_LEVERAGE = {
    "ADANIGREEN": 3.3852404277625494, "ADANIENSOL": 3.3333333333333335,
    "ADANIENT": 3.8095235799469624, "ADANIPOWER": 3.3333333333333335,
    "ETERNAL": 3.9432176656151423, "LODHA": 3.7285607755406414,
    "TMCV": 3.3411293286258257, "VEDL": 2.380952380952381,
    "ADANIPORTS": 4.0, "CGPOWER": 4.046944556859572,
    "MAXHEALTH": 4.166666666666667, "CANBK": 4.304778155680283,
    "SHRIRAMFIN": 3.9525691699604746, "TRENT": 3.8654813575520484,
    "JIOFIN": 4.3478260869565215,
}
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "ema_regime_crossover_swing_mtf_leverage")
...
for _, cand in trades_df.iterrows():
    leverage = MTF_LEVERAGE.get(cand["symbol"], 1.0)
    buying_power = capital_per_trade * leverage
    qty = int(buying_power // cand["entry_price"])
    if qty <= 0:
        continue
    position_value = qty * cand["entry_price"]
    borrowed = max(0.0, position_value - capital_per_trade)
    interest_cost = borrowed * MTF_INTEREST_PCT_PA * (cand["hold_days"] / 365.0)
    ...
    t.update(charges)
    t["leverage"] = leverage
    t["interest_cost"] = round(interest_cost, 2)
    t["net_pnl_rupees"] = round(pnl_rupees - charges["total_charges"] - interest_cost, 2)
    t["group"] = grp
    accepted.append(t)
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_swing_mtf_leverage_trades.csv"), index=False)
...
if result_df.empty:
    ...
    total_interest = result_df["interest_cost"].sum()
    print(f"MTF interest: Rs {total_interest:,.0f}  ({total_interest/total_gross*100 if total_gross else float('nan'):.1f}% of gross P&L)  "
          f"| avg leverage used: {result_df['leverage'].mean():.2f}x")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_swing_cnc_concentration_v3_tightstop_trades.csv")
