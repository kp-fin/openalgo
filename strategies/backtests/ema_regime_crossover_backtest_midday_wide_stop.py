"""
EMA Regime Crossover -- Combined: Midday Entry Window + Wide Stop
...
Four single-variable variants were tested independently on 2026-07-29:
  - Baseline (live config):        WR(net) 38.1%, Sharpe(net) -1.75, maxDD 192.0%
  - Wide stop (2.2xATR):           WR(net) 41.1%, Sharpe(net) -1.84, maxDD 207.7%
  - Midday entry (11:30-13:30):    WR(net) 40.1%, Sharpe(net) -1.42 (best), maxDD 65.9% (best)
  - Gap-aligned entries:           WR(net) 37.2%, Sharpe(net) -1.88 (worst) -- REJECTED

Midday entry won on Sharpe/drawdown (fewer trades -> less cost drag and
variance, not genuinely better per-trade quality -- WR/PF barely moved).
Wide stop won on win rate (fewer forced stop-outs) but made Sharpe worse
(losses just moved into REVERSE_CROSS instead, with more day-to-day
variance). This variant combines the two best individual levers -- entries
restricted to 11:30-13:30 IST AND STOP_ATR_MULT widened to 2.2 -- to test
whether they compound favourably. Neither lever alone reached the +1.5
Sharpe goal; this is the last filter/parameter iteration before concluding
the entry signal itself needs a structural redesign rather than another
variant.
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_midday_wide_stop")
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_midday_wide_stop_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Midday Window (11:30-13:30) + Wide Stop (2.2xATR) ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_midday_wide_stop_trades.csv")
