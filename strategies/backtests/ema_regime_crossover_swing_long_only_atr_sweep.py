"""
EMA Regime Crossover — SWING, LONG-ONLY — ATR stop/target multiple sweep

RS_MOMENTUM_SLOPE_LOOKBACK locked at 2 (best point from the earlier lookback
sweep: PF 1.40, +Rs 36.93L, 16/20 symbols profitable -- see
ema_regime_crossover_swing_long_only_rs_lookback_sweep.py). This script
fetches + precomputes each symbol's regime/signals ONCE (identical across
every combo, since regime doesn't depend on the stop/target multiples), then
re-runs only the entry/exit simulation across a stop x target ATR-multiple
grid. Karan's request 2026-07-19.

Original multiples (1.36x stop / 3.0x target, ~2.2:1 reward:risk) came from
the intraday EMA Regime Crossover spec -- untested for whether they're
actually well-suited to this swing variant's much longer average holding
period (~12-13 calendar days vs. same-day intraday).
"""
...
RS_ZSCORE_WINDOW = 10
RS_SMOOTH_WINDOW = 2
RS_MOMENTUM_SLOPE_LOOKBACK = 2   # locked in from the earlier sweep

STOP_MULTS = [1.0, 1.36, 1.75, 2.25, 2.75]
TARGET_MULTS = [2.0, 2.5, 3.0, 3.5, 4.0, 5.0, 6.0, 7.0, 8.0]
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_swing_long_only_atr_sweep")
...
# ---- Fetch + precompute everything (regime is now fixed at LOOKBACK=2, shared across every combo) ----
index_data = {}
...
print(f"Universe: {len(UNIVERSE)} names (LONG-ONLY, LOOKBACK=2 locked, sweeping ATR stop/target)")
symbol_data = {}
skipped = {}
...
for stop_mult in STOP_MULTS:
    for target_mult in TARGET_MULTS:
        ...
        if summary is None:
            print(f"stop={stop_mult} target={target_mult}: no trades")
            continue
        summary_rows.append(summary)
        print(f"stop={stop_mult:>4} target={target_mult:>4} | trades={summary['trades']:>4} | "
              f"WR={summary['win_rate']:>5.1f}% | PF={summary['profit_factor']:>5.2f} | "
              f"avg={summary['avg_pnl_pct']:>+7.3f}% | total=Rs {summary['total_rupees']:>+13,.0f} | "
              f"profitable={summary['profitable_symbols']} | avg_hold={summary['avg_holding_days']}d")

summary_df = pd.DataFrame(summary_rows)
summary_df.to_csv(os.path.join(OUT_DIR, "atr_sweep_summary.csv"), index=False)
print(f"\nSummary written to {os.path.join(OUT_DIR, 'atr_sweep_summary.csv')}")
print(f"Skipped symbols: {skipped}")

print("\nTop 5 by profit factor:")
print(summary_df.sort_values("profit_factor", ascending=False).head(5).to_string(index=False))
print("\nTop 5 by total P&L:")
print(summary_df.sort_values("total_rupees", ascending=False).head(5).to_string(index=False))
