"""
EMA Regime Crossover -- Narrow Window (11:45-12:45) + Wide Stop
...
Five variants tested so far on 2026-07-29, in order:
  - Baseline (live config):              WR(net) 38.1%, Sharpe(net) -1.75, maxDD 192.0%
  - Wide stop (2.2xATR):                 WR(net) 41.1%, Sharpe(net) -1.84, maxDD 207.7%
  - Midday entry (11:30-13:30):          WR(net) 40.1%, Sharpe(net) -1.42, maxDD 65.9%
  - Gap-aligned entries:                 WR(net) 37.2%, Sharpe(net) -1.88 -- REJECTED
  - Midday (11:30-13:30) + wide stop:    WR(net) 42.4%, Sharpe(net) -1.23 (best so far), maxDD 64.5%

The combined variant showed both WR and Sharpe improving together as the
window narrowed and the stop widened -- not noise, a consistent trend
across three consecutive tests (Sharpe -1.75 -> -1.42 -> -1.23; WR 38.1%
-> 40.1% -> 42.4%). This variant pushes the same direction further:
narrows the entry window from the prior 2-hour band (11:30-13:30) to a
tighter 1-hour band (11:45-12:45), centered on 12:00 -- the single hour
that showed net-positive P&L in the original per-hour diagnostic (out of
all 20-name-universe candidates, hour=12 alone: WR 43.4%, net +Rs 10,511,
every other hour flat-to-negative). STOP_ATR_MULT stays at 2.2 (unchanged
from the prior combined variant) to isolate the window-narrowing effect.

Everything else (regime, entry signal, universe, window, sizing, both
circuit-breaker mechanics, charges model) is identical to the current live
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_narrow_window_wide_stop")
...
print(f"\nSignal candidates surviving entry-window filter: {len(trades_df)} (rejected outside window: {rejected_window})")
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_wide_stop_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Narrow Window (11:45-12:45) + Wide Stop (2.2xATR) ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_wide_stop_trades.csv")
