"""
EMA Regime Crossover -- Midday-Entry-Window Variant (11:30-13:30 IST only)
...
grouping net-of-charges P&L by entry hour showed only the 12:00 hour was
net-profitable after charges (+Rs 10,511); every other hour was flat to
negative (worst: 10:00 at -Rs 97,852). Two plausible mechanisms: (a) early
entries (9-11am) eat more noise-driven whipsaw stops right after the open,
before a real trend has established; (b) late entries (14-15) run out of
time to reach target or a profitable hard-exit before the 15:00 cutoff.
Midday entries may sit in a sweet spot -- past the opening noise, with
still-enough time before hard exit.

This variant restricts new entries to a widened window around the single
profitable hour (11:30-13:30 IST) rather than the literal 12:00-12:59
bucket alone, to avoid overfitting to one hour's sample (n=927) in
isolation. Independent, single-variable test from the wide-stop variant
(ema_regime_crossover_backtest_wide_stop.py) tested in parallel --
STOP_ATR_MULT stays at the original 1.36xATR here.
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_midday_entry")
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_midday_entry_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Midday Entry Window (11:30-13:30) ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_midday_entry_trades.csv")
