"""
EMA Regime Crossover — SWING, LONG-ONLY — RS_MOMENTUM_SLOPE_LOOKBACK sweep

Same strategy as ema_regime_crossover_swing_long_only_backtest.py (daily
bars, RS-Ratio/RS-Momentum regime, long-only, no time-based exit) -- this
script fetches each symbol's data ONCE, then re-evaluates the "moving
towards acceleration" lookback (RS_MOMENTUM_SLOPE_LOOKBACK: how many days
back RS-Momentum is compared against to call it "rising") across several
candidate values, since that's the one free parameter in the regime
definition that hasn't been tuned yet. RS_ZSCORE_WINDOW/RS_SMOOTH_WINDOW
(10/2, JdK published defaults) are left fixed -- refetching per lookback
value would be wasteful since only the regime's rising/falling test changes.

Karan's request 2026-07-19, following the long-only result (342 trades,
WR 36.8%, PF 1.34, 16/20 symbols profitable at LOOKBACK=3).
"""
...
RS_ZSCORE_WINDOW = 10
RS_SMOOTH_WINDOW = 2
LOOKBACK_CANDIDATES = [1, 2, 3, 5, 7, 10, 15, 20]
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_swing_long_only_sweep")
...
# ---- Fetch + precompute everything that does NOT depend on the sweep parameter ----
index_data = {}
...
print(f"Universe: {len(UNIVERSE)} names (LONG-ONLY, sweeping RS_MOMENTUM_SLOPE_LOOKBACK)")
symbol_data = {}
skipped = {}
...
def run_backtest(lookback):
    ...
    for symbol, df in symbol_data.items():
        rs_momentum_rising = df["rs_momentum"] > df["rs_momentum"].shift(lookback)
        bull_regime = (df["rs_ratio"] > 100) & ((df["rs_momentum"] > 100) | rs_momentum_rising)
        regime = np.where(bull_regime, "BULL", "NOT_BULL")

        n = len(df)
        for i in range(n - 1):
            if not (df["bull_cross"].iloc[i] and regime[i] == "BULL"):
                continue
    ...
    return {
        "lookback": lookback, "trades": n, "win_rate": round(wr, 1),
...
for lb in LOOKBACK_CANDIDATES:
    res = run_backtest(lb)
    if res is None:
        print(f"LOOKBACK={lb}: no trades")
        continue
    summary, result_df = res
    summary_rows.append(summary)
    result_df.to_csv(os.path.join(OUT_DIR, f"trades_lookback_{lb}.csv"), index=False)
    print(f"LOOKBACK={lb:>2}: trades={summary['trades']:>4} | WR={summary['win_rate']:>5.1f}% | "
          f"PF={summary['profit_factor']:>5.2f} | avg={summary['avg_pnl_pct']:>+7.3f}% | "
          f"total=Rs {summary['total_rupees']:>+13,.0f} | profitable_symbols={summary['profitable_symbols']}")

summary_df = pd.DataFrame(summary_rows)
summary_df.to_csv(os.path.join(OUT_DIR, "lookback_sweep_summary.csv"), index=False)
print(f"\nSummary written to {os.path.join(OUT_DIR, 'lookback_sweep_summary.csv')}")
