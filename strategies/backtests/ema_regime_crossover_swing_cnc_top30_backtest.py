"""
EMA Regime Crossover -- Swing CNC, Middle-Ground Universe Size Test (Top-30
from Full Nifty 100 Pool) (2026-08-01, Karan-requested follow-on)

SECOND DATA POINT for the middle-ground-size question, run alongside the
top-25 variant (`ema_regime_crossover_swing_cnc_top25_backtest.py`, same
day) -- see that script's docstring for full campaign context. This is the
same test at N=30 instead of N=25, everything else (ranking method, source
pool, regime/entry/exit rules, concentration cap, charges, window)
identical.

CONTEXT: this campaign has now tested three universe-size extremes for the
deployed EMA Swing CNC strategy (live/paper since 2026-07-31):

1. Static 15-name high-beta subset ranked from a 59-name pool ("Run 3",
   ...
   DEPLOYED.
2. Rolling universe (monthly/quarterly re-rank of the 59-name pool):
   REJECTED, Sharpe(net) ~2.0-2.1.
3. Full Nifty 100, unranked, all 100 names traded directly
   (`ema_regime_crossover_swing_cnc_nifty100_backtest.py`): REJECTED
   decisively, Sharpe(net) collapsed to 0.55. Root cause diagnosed as
   concurrency-cap dilution: with 100 names competing for only
   MAX_CONCURRENT=6 slots, the raw concurrency cap became the binding
   constraint instead of trade quality (only 1 concentration-cap rejection
   fired, vs. Run 3's 11 -- proof the concentration cap was not even being
   exercised, because the concurrency cap was rejecting candidates first).
4. Top-15 RE-RANKED from the full Nifty 100 pool instead of the 59-name pool
   (`ema_regime_crossover_high_beta_rank_nifty100.py` for ranking,
   `ema_regime_crossover_swing_cnc_high_beta_nifty100_backtest.py` for the
   backtest) -- still only 15 names traded, just a better-ranked 15: HELPS,
   Sharpe(net) 2.76->3.72, PF(net) 1.67->2.26, WR(net) 34.9%->37.4%,
   maxDD 18.1%->10.6%, 147 trades. Confirms the wider pool contains
   genuinely better names than the original 59-name pool. Karan has NOT yet
   decided whether to deploy this list -- flagged as a candidate only.

THIS SCRIPT asks the one remaining untested angle: a "middle ground"
universe SIZE, ~25-30 names, between the tested extremes of 15 and 100.
Does the concurrency-cap-dilution mechanism that killed the 100-name test
already bite at 25 names, or does it only kick in at a much larger size?

METHOD: reuses the EXACT same causal volatility-ranking methodology and
source pool as test #4 above (`ema_regime_crossover_high_beta_rank_nifty100.py`
-- full Nifty 100 pool, ann. realized vol of daily close-to-close log
returns, 2021-07-01->2026-06-30, causal/full-sample, no lookahead) --
just a LARGER top-N slice (top 30) instead of top-15. Ranking CSV:
`ema_regime_crossover_high_beta_rank_nifty100/high_beta_ranking_nifty100.csv`.

TOP-30 UNIVERSE (by descending ann_vol_pct):
ADANIGREEN, ADANIENSOL, MAZDOCK, MOTHERSON, ADANIENT, ADANIPOWER, ETERNAL,
LODHA, TMCV, IRFC, UNIONBANK, VEDL, ADANIPORTS, HINDZINC, RECLTD, CGPOWER,
SOLARINDS, PFC, ENRIN, JINDALSTEL, MAXHEALTH, PNB, CANBK, SHRIRAMFIN, TRENT,
VBL, CHOLAFIN, AMBUJACEM, HAL, BANKBARODA.

CONCENTRATION CAP: MAX_PER_GROUP=2, UNCHANGED from Run 3 -- run this as the
SECOND data point, apples-to-apples vs. Run 3 and the top-25 variant. The
top-30 list contains 6 Adani-group names (adds AMBUJACEM, acquired by Adani
in 2022, to the top-25's 5), 2 Vedanta-group (VEDL, HINDZINC), and 2
Tata-group (TMCV, TRENT), so the group cap has real names to constrain here,
unlike the diluted full-100 test. GROUP_OF is reused verbatim (the relevant
entries) from the already-extended map in
`ema_regime_crossover_swing_cnc_nifty100_backtest.py`. The script prints
BOTH the concurrency-cap rejection count and the concentration-cap rejection
count explicitly, since distinguishing which one is binding is the entire
point of this diagnostic (see module context above and the full writeup in
`equities-system/strategies/ema_regime_crossover.md`).
...
2.2xATR, 40-trading-day max-hold backstop, MAX_CONCURRENT=6 (unchanged --
not the variable under test here), ASSUMED_LEVERAGE=1,
capital_per_trade = ALLOCATED_CAPITAL / MAX_CONCURRENT,
ALLOCATED_CAPITAL=250,000, same CNC delivery charges model (incl. DP
charge), same 2021-07-01->2026-06-30 window. This is backtest-only research
-- the deployed live/paper script and strategy_configs.json are untouched.
"""
...
MAX_CONCURRENT = 6           # NOT the variable under test here -- unchanged
ASSUMED_LEVERAGE = 1  # CNC requires full payment -- no MIS-style leverage assumption

# ---- Concentration cap -- UNCHANGED FROM RUN 3 (primary test, apples-to-apples) ----
MAX_PER_GROUP = 2

# ---- Universe: top-30 by ann. realized vol, ranked from the FULL Nifty 100 pool ----
# Source: ema_regime_crossover_high_beta_rank_nifty100/high_beta_ranking_nifty100.csv
TOP_30 = [
    "ADANIGREEN", "ADANIENSOL", "MAZDOCK", "MOTHERSON", "ADANIENT", "ADANIPOWER",
    "ETERNAL", "LODHA", "TMCV", "IRFC", "UNIONBANK", "VEDL", "ADANIPORTS",
    "HINDZINC", "RECLTD", "CGPOWER", "SOLARINDS", "PFC", "ENRIN", "JINDALSTEL",
    "MAXHEALTH", "PNB", "CANBK", "SHRIRAMFIN", "TRENT",
    "VBL", "CHOLAFIN", "AMBUJACEM", "HAL", "BANKBARODA",
]
UNIVERSE = TOP_30

# Issuer/sector group map -- reused verbatim (relevant entries only) from the
# already-extended map in ema_regime_crossover_swing_cnc_nifty100_backtest.py.
# Names not listed default to their own singleton group via group_of()'s
# fallback, same convention as Run 3 and the full-100 test.
GROUP_OF = {
    ...
    "TMCV": "TATA_GROUP", "TRENT": "TATA_GROUP",
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "ema_regime_crossover_swing_cnc_top30")
...
print(f"Universe: {len(UNIVERSE)} names (top-30 by ann. realized vol, ranked from full Nifty 100 pool) | "
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_swing_cnc_top30_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Swing CNC, Top-30 (from Nifty 100 pool) Universe "
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_swing_cnc_top30_trades.csv")
