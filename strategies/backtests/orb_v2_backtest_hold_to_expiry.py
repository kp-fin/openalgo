"""
ORB_Spread — Hold to Expiry Variant (exploratory)
Nifty 50 Spot | 2021-07-01 -> 2026-06-30

Karan asked (exploratory, 2026-08-06): what if ORB_Spread positions are held
to expiry instead of force-closed same-day at 15:15? This tests that as a
backtest-only variant -- no live script changes, no vault-spec changes.
...
Exit logic changes from the current live design:
  - Same +40pt target / -25pt stop, checked on 5m closes -- but instead of a
    same-day 15:15 HARD_EXIT, the walk-forward continues day-by-day (using
    each day's 09:15-15:30 5m bars) until that week's expiry.
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "orb_v2_hold_to_expiry")
...
print("\n" + "=" * 90)
print("HOLD TO EXPIRY — spot points (naked proxy)")
print("=" * 90)
...
print("\n" + "=" * 90)
print(f"HOLD TO EXPIRY — {SPREAD_WIDTH}pt/{SPREAD_COST_PCT*100:.0f}% spread overlay (same convention as adopted same-day model)")
print("=" * 90)
