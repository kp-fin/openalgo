"""
HH-HL Pullback Breakout (Flag Continuation) -- Nifty 100, PORTFOLIO-CONSTRAINED
...
PORTFOLIO PARAMETERS -- reused byte-for-byte from EMA Swing-59/100's proven
config rather than invented fresh, since this is the same asset class (CNC
equity swing, comparable holding periods) and no evidence yet justifies a
different setting for this pattern: ALLOCATED_CAPITAL=Rs 2,50,000,
MAX_CONCURRENT=6, MAX_PER_GROUP=2, capital_per_trade = ALLOCATED_CAPITAL /
MAX_CONCURRENT, ASSUMED_LEVERAGE=1 (CNC, no leverage).
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hh_hl_pullback_breakout_nifty100_portfolio")
...
trades_df.to_csv(os.path.join(OUT_DIR, "hh_hl_pullback_breakout_nifty100_portfolio_trades.csv"), index=False)
...
print(f"\n=== HH-HL Pullback Breakout -- Nifty 100, PORTFOLIO-CONSTRAINED "
...
print(f"\nTrade log -> {OUT_DIR}/hh_hl_pullback_breakout_nifty100_portfolio_trades.csv")
