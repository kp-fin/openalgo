"""
Real Dhan MIS (intraday) leverage survey — Nifty 100 (Nifty 50 + Nifty Next 50)

Same method as mtf_leverage_survey.py, for MIS instead of MTF -- Karan asked
to do the same check for intraday leverage after the MTF survey showed the
Sandbox engine's flat placeholder (2x for MTF) was well off real per-symbol
data. The existing equity_mis_leverage default (5x, set well before MTF
support existed) was never checked against real Dhan MIS margin data either.

Method: fetch each symbol's live LTP (multiquotes), then call /api/v1/margin
with a 1-share BUY, product=MIS, pricetype=MARKET at that LTP. Dhan's real
...
def get_mis_margin(symbol, ltp):
    """Real Dhan margin calculator, product=MIS, 1-share BUY at current LTP."""
    r = requests.post(
        ...
        json={
            ...
            "positions": [{
                ...
                "quantity": "1", "product": "MIS", "pricetype": "MARKET", "price": str(ltp),
            }],
        },
        timeout=15,
    )
...
for symbol in UNIVERSE:
    ...
    if ltp is None:
        ...
        continue
    try:
        resp = get_mis_margin(symbol, ltp)
...
df.to_csv(os.path.join(OUT_DIR, "mis_leverage_survey.csv"), index=False)

valid = df.dropna(subset=["implied_leverage"])
print(f"\n=== MIS Leverage Survey: {len(valid)}/{len(df)} names succeeded ===")
if not valid.empty:
