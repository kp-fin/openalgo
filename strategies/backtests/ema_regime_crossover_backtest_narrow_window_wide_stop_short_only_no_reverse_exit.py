"""
EMA Regime Crossover -- Narrow Window + Wide Stop + SHORT-only, no REVERSE_CROSS exit
...
Best result so far in this iteration (narrow window 11:45-12:45 + wide stop
2.2xATR + SHORT-only + 59-name universe): WR(net) 44.7%, Sharpe(net) -0.24,
PF(net) 0.96, maxDD 29.8%. That run's own exit-reason breakdown showed the
two loss buckets are REVERSE_CROSS (n=667, WR 0%, -Rs 440k) and STOP (n=212,
WR 0%, -Rs 320k), while HARD_EXIT (WR 62.1%, +Rs 264k) and TARGET (100% WR
by construction, +Rs 462k) are strong. REVERSE_CROSS exits a trade the
moment the opposite EMA cross fires, before stop or target -- Karan asked
whether removing that early exit and letting trades ride to STOP, TARGET,
or HARD_EXIT instead helps (more room to recover) or hurts (lets losers run
further before the eventual stop/hard-exit).
...
charges model) is identical to the current-best variant for a clean
comparison -- only the exit logic changes.
"""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_narrow_window_wide_stop_short_only_no_reverse_exit")
...
print(f"Universe: {len(UNIVERSE)} names | Entry window: {ENTRY_WINDOW_START} - {ENTRY_WINDOW_END} IST only | "
      f"SHORT_ONLY={SHORT_ONLY} | EXIT_ON_REVERSE_CROSS={EXIT_ON_REVERSE_CROSS}")
all_trades = []
skipped = {}
rejected_window = 0
        ...
        for ts2, bar2 in rest.iterrows():
            ...
            if direction == "LONG":
                ...
                if bar2["high"] >= target_px:
                    ...
                    break
                if EXIT_ON_REVERSE_CROSS and bar2["bear_cross"]:
                ...
                if bar2["low"] <= target_px:
                    exit_px, exit_ts, reason = target_px, ts2, "TARGET"
                    break
                if EXIT_ON_REVERSE_CROSS and bar2["bull_cross"]:
                    exit_px, exit_ts, reason = bar2["close"], ts2, "REVERSE_CROSS"
                    break
            if t >= HARD_EXIT:
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_wide_stop_short_only_no_reverse_exit_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_wide_stop_short_only_no_reverse_exit_trades.csv")
