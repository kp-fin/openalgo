"""
HH-HL Pullback Breakout (Flag Continuation) -- Nifty 100, PORTFOLIO-CONSTRAINED, MAX_CONCURRENT=10
(2026-08-03, follow-on to the signal-level Run 1 result at Karan's request)
...
THIS RUN adds a portfolio-constraint layer on top of the signal-level Run 1
result (58 trades, WR(net) 50.0%, PF(net) 2.23 on this same Nifty 100
...
raw signal candidates first (unchanged logic from Run 1), then walk them
...
PORTFOLIO PARAMETERS -- MAX_CONCURRENT raised from 6 to 10 in this variant.
The MAX_CONCURRENT=6 run (hh_hl_pullback_breakout_nifty100_portfolio_backtest.py)
rejected 15 of 58 raw signal candidates purely for lack of a free concurrency
slot -- this pattern holds much longer (52-105 trading days) than EMA
Swing-59/100 (whose 6-slot config the original run just reused unchanged),
so a higher cap was worth testing before treating the MC=6 Sharpe figure as
settled. Everything else unchanged: ALLOCATED_CAPITAL=Rs 2,50,000,
MAX_PER_GROUP=2, capital_per_trade = ALLOCATED_CAPITAL / MAX_CONCURRENT,
ASSUMED_LEVERAGE=1 (CNC, no leverage). Note capital_per_trade shrinks as
MAX_CONCURRENT rises (Rs 41,666 -> Rs 25,000 per slot), so this is a genuine
concurrency-vs-per-trade-size trade-off, not a free lunch.

CONCURRENCY-SLOT SIMPLIFICATION: a trade occupies one full concurrency/group
slot from entry until its FINAL exit (i.e. until leg 2 resolves), even
though only half the position's capital is actually still at risk after leg
1 books at 1:2 RR. This slightly understates real capital efficiency (a
partial exit could in principle free up room for a new trade) but matches
the single-exit assumption every other portfolio-constrained script in this
vault uses -- documented simplification, not an oversight.

GROUP MAP: extends the GROUP_OF map from
ema_regime_crossover_swing_cnc_nifty100_backtest.py with additional real
parent/promoter relationships visible once this pattern's own trade list is
examined (ICICI Group, Max Group, SBI Card, JSW Group beyond just JSWSTEEL,
Godrej Group, plus a few more Tata/Mahindra/L&T subsidiaries not in the
original map). See GROUP_OF definition below for the itemised additions and
rationale for each.
...
# Issuer/sector group map -- base map (Adani/Tata/Bajaj/Birla/Mahindra/Vedanta/
# L&T/HDFC/SBI/Reliance) reused byte-for-byte from
# ema_regime_crossover_swing_cnc_nifty100_backtest.py, plus additional real
# parent/promoter groupings not needed there but relevant to this pattern's
# own trade list:
# - ICICI_GROUP: ICICIBANK (base map had it as a singleton -- no sibling
#   ICICI entity was in that script's trade list to warrant grouping; adding
#   here defensively even though ICICIGI/ICICIAMC are Midcap-100-only names
#   not in THIS Nifty-100 universe -- harmless no-op for this run, kept for
#   parity with the Nifty 200 portfolio script)
# - MAX_GROUP: MAXHEALTH + MFSL (Max Financial Services) -- MFSL is
#   Midcap-100-only, same parity note as above
# - JSW_GROUP: JSWSTEEL + JSWENERGY -- real single conglomerate (distinct
#   from Jindal Steel & Power, per the base map's existing JSW-vs-Jindal
#   note); JSWENERGY is Midcap-100-only, parity note as above
# - GODREJ_GROUP: GODREJCP + GODREJPROP -- GODREJPROP is Midcap-100-only
GROUP_OF = {
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hh_hl_pullback_breakout_nifty100_portfolio_mc10")
...
# Chronological event simulation, same convention as
# ema_regime_crossover_swing_cnc_nifty100_backtest.py: process exits due
# before a candidate's entry time before evaluating that candidate. A trade
# occupies its concurrency/group slot from entry until FINAL exit (see
# module docstring's "CONCURRENCY-SLOT SIMPLIFICATION" note).
pending_exits = []
seq = 0
open_positions = {}
group_open_count = {}
...
trades_df.to_csv(os.path.join(OUT_DIR, "hh_hl_pullback_breakout_nifty100_portfolio_mc10_trades.csv"), index=False)
...
print(f"\n=== HH-HL Pullback Breakout -- Nifty 100, PORTFOLIO-CONSTRAINED, MC=10 "
...
# Daily-return Sharpe: allocate net P&L to its final-exit date, same
# convention as every other portfolio-constrained backtest in this vault.
...
print(f"\nTrade log -> {OUT_DIR}/hh_hl_pullback_breakout_nifty100_portfolio_mc10_trades.csv")
