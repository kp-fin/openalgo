"""
EMA Regime Crossover -- VIX-Triggered Position Cap (max concurrent 6 -> 4)

Isolated ablation against the resized baseline (ema_regime_crossover_backtest_resized.py):
only the portfolio-level concurrency cap becomes conditional on India VIX, everything
else (regime, entry, original exit mechanics -- no trailing-target, no daily-loss
circuit breaker -- universe, window, current sizing) is unchanged. Tested separately
from ema_regime_crossover_backtest_daily_circuit_breaker.py so each risk control's
individual effect on drawdown is isolated, not conflated.

Rule (Karan-confirmed 2026-07-28): use the PREVIOUS trading day's completed India VIX
close (INDIAVIX, NSE_INDEX daily bars -- causal, no lookahead into today's still-forming
value). If prior-day VIX close > 20, MAX_CONCURRENT drops to 4 for today; otherwise stays
at 6. 20 is the standard "elevated" threshold for India VIX (below ~15 calm, 15-20 normal,
above 20 is where options markets start pricing real stress).

Same drawdown metric (peak-to-trough on the cumulative-P&L equity curve) computed as for
the resized baseline (max DD Rs 1,72,073 / 68.8% of Rs 2,50,000 allocated capital,
2021-08-04 -> 2023-12-14) for a direct before/after comparison.
"""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_vix_cap")
...
def _fetch(symbol, exchange, interval):
    try:
        resp = client.history(symbol=symbol, exchange=exchange, interval=interval,
...
# ---- VIX day-cap lookup (causal: previous day's completed close) ----
print("Fetching India VIX (daily) ...")
vix_df, vix_err = _fetch("INDIAVIX", "NSE_INDEX", "D")
if vix_err:
    raise SystemExit(f"Could not fetch INDIAVIX: {vix_err}")
vix_df = vix_df.copy()
vix_df.index = vix_df.index.normalize()
vix_df["prev_close"] = vix_df["close"].shift(1)
day_cap = {}
for d, row in vix_df.iterrows():
    day_cap[d.date()] = MAX_CONCURRENT_HIGH_VIX if (pd.notna(row["prev_close"]) and row["prev_close"] > VIX_THRESHOLD) else MAX_CONCURRENT_NORMAL
high_vix_days = sum(1 for v in day_cap.values() if v == MAX_CONCURRENT_HIGH_VIX)
print(f"VIX days: {len(day_cap)} total, {high_vix_days} high-VIX days (cap=4), "
      f"{len(day_cap) - high_vix_days} normal days (cap=6)\n")
...
for symbol in UNIVERSE:
    df15, err15 = _fetch(symbol, "NSE", "15m")
    if err15:
...
# ---- Portfolio-level simulation: VIX-conditional concurrency cap ----
capital_per_trade = ALLOCATED_CAPITAL * POSITION_PCT
buying_power = capital_per_trade * ASSUMED_MIS_LEVERAGE
open_positions = []
accepted = []
rejected_capacity = 0
default_cap = MAX_CONCURRENT_NORMAL  # for any date missing from the VIX series (shouldn't happen given full coverage)

for _, trade in trades_df.iterrows():
    open_positions = [p for p in open_positions if p > trade["entry_time"]]
    todays_cap = day_cap.get(trade["entry_time"].date(), default_cap)
    if len(open_positions) >= todays_cap:
        rejected_capacity += 1
        continue
    open_positions.append(trade["exit_time"])
    qty = int(buying_power // trade["entry_price"])
    pnl_rupees = qty * trade["entry_price"] * trade["pnl_pct"]
    t = trade.to_dict()
    t["qty"] = qty
    t["pnl_rupees"] = round(pnl_rupees, 2)
    t["day_cap"] = todays_cap
    accepted.append(t)

result_df = pd.DataFrame(accepted)
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_vix_cap_trades.csv"), index=False)

print(f"Skipped (no data): {skipped}")
print(f"Rejected for exceeding the day's cap (4 or 6 depending on VIX): {rejected_capacity}")
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- VIX-Triggered Cap Backtest ===")
    ...
    print(f"Trades taken on high-VIX (cap=4) days: {(result_df['day_cap']==4).sum()} "
          f"({(result_df['day_cap']==4).mean()*100:.1f}%)")
