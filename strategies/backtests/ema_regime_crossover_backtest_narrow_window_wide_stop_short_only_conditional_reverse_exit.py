"""
EMA Regime Crossover -- Narrow Window + Wide Stop + SHORT-only, conditional REVERSE_CROSS exit
...
Test #8 (narrow window 11:45-12:45 + wide stop 2.2xATR + SHORT-only +
59-name universe) is the best result so far: WR(net) 44.7%, Sharpe(net)
-0.24, PF(net) 0.96, maxDD 29.8%. Test #9 tried removing REVERSE_CROSS as
an exit trigger entirely -- REJECTED: WR(net) rose to 48.1% but Sharpe(net)
fell to -0.46 and maxDD widened to 35.2%, because trades that used to be
cut early and cheaply by REVERSE_CROSS instead ran on to the wider stop,
concentrating losses into fewer/larger losing trades.

This variant is the softer version Karan asked for: only skip the
REVERSE_CROSS exit if the trade is currently favourable (unrealized P&L
> 0 at the close of the bar where the opposite EMA cross fires) -- i.e.
let winners run past the reverse signal toward TARGET/HARD_EXIT, but still
cut losers immediately on REVERSE_CROSS exactly as test #8 did. This
should keep REVERSE_CROSS's risk-cutting role on losing trades (which
test #9 showed matters) while capturing whatever extra upside exists on
trades that are already working when the reverse signal fires.
...
charges model) is identical to test #8 for a clean comparison -- only the
REVERSE_CROSS exit condition changes.
"""
...
SHORT_ONLY = True
CONDITIONAL_REVERSE_EXIT = True  # skip REVERSE_CROSS exit only if trade is currently favourable
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_narrow_window_wide_stop_short_only_conditional_reverse_exit")
...
print(f"Universe: {len(UNIVERSE)} names | Entry window: {ENTRY_WINDOW_START} - {ENTRY_WINDOW_END} IST only | "
      f"SHORT_ONLY={SHORT_ONLY} | CONDITIONAL_REVERSE_EXIT={CONDITIONAL_REVERSE_EXIT}")
all_trades = []
skipped = {}
rejected_window = 0
        ...
        for ts2, bar2 in rest.iterrows():
            ...
            if direction == "LONG":
                ...
                if bar2["high"] >= target_px:
                    ...
                    break
                if bar2["bear_cross"]:
                    favourable = bar2["close"] > entry_price
                    if not (CONDITIONAL_REVERSE_EXIT and favourable):
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_wide_stop_short_only_conditional_reverse_exit_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_wide_stop_short_only_conditional_reverse_exit_trades.csv")
