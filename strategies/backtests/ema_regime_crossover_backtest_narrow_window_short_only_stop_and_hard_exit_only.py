"""
EMA Regime Crossover -- Narrow Window + SHORT-only, STOP + HARD_EXIT only (no TARGET, no REVERSE_CROSS)
...
3.0xATR + REVERSE_CROSS + SHORT-only + 59-name universe) is the best
result so far: WR(net) 44.7%, Sharpe(net) -0.24, PF(net) 0.96, maxDD 29.8%.
Four exit-logic variants in a row (tests #9-#12: removing REVERSE_CROSS,
conditional REVERSE_CROSS, a trailing stop, a partial-profit-take at
target) all failed to beat it -- every attempt to modify the
stop/target/reverse-cross skeleton traded a bigger cleaner edge for a
smaller noisier one. Karan asked (2026-07-30) to try a structurally
different exit model rather than another variant on the same skeleton.

Diagnostic that motivates this variant: across EVERY test run so far
(baseline through test #12), HARD_EXIT -- trades that survive to 15:00
untouched by any earlier exit -- has consistently been the single
best-performing exit bucket by win rate and average P&L per trade (test #8:
WR 62.1%, avg +0.245%/trade; test #12's HARD_EXIT WR 68.8%, avg
+0.444%/trade). TARGET caps a trade's upside at a fixed 3.0xATR the moment
it's reached, and REVERSE_CROSS has been shown (test #10) to only ever
fire at a loss for this signal. Both remove trades from the pool before
they can become the kind of trade that reaches HARD_EXIT.

This variant tests the logical extreme of that diagnostic: remove TARGET
and REVERSE_CROSS entirely. The ONLY two ways a trade can exit are the
fixed 2.2xATR stop (real risk control -- cannot be removed) or the 15:00
IST hard close. No price-level profit cap, no indicator-based early exit.

Everything else (regime, entry signal, window, fixed stop multiple,
SHORT-only, 59-name universe, sizing, both circuit-breaker mechanics,
charges model) is identical to test #8 -- only the exit signal set changes.
"""
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ema_regime_crossover_narrow_window_short_only_stop_and_hard_exit_only")
...
print(f"Universe: {len(UNIVERSE)} names | Entry window: {ENTRY_WINDOW_START} - {ENTRY_WINDOW_END} IST only | "
      f"SHORT_ONLY={SHORT_ONLY} | exits: STOP({STOP_ATR_MULT}xATR) + HARD_EXIT only")
all_trades = []
skipped = {}
rejected_window = 0
...
result_df.to_csv(os.path.join(OUT_DIR, "ema_regime_crossover_narrow_window_short_only_stop_and_hard_exit_only_trades.csv"), index=False)
...
if result_df.empty:
    ...
    print(f"\n=== EMA Regime Crossover -- Narrow Window + SHORT-only, STOP + HARD_EXIT only ===")
    ...
    print(f"\nTrade log -> {OUT_DIR}/ema_regime_crossover_narrow_window_short_only_stop_and_hard_exit_only_trades.csv")
