"""
EMA Regime Crossover — Swing CNC — HIGH-BETA TOP-15 RE-RANKED FROM FULL NIFTY 100
POOL — PARALLEL FORWARD-TEST CANDIDATE (built 2026-08-01, DEPLOYED IN PARALLEL
alongside the currently-deployed Swing CNC strategy, NOT a replacement for it)

*** THIS IS A FORWARD-TEST OF THE UNIVERSE CHANGE ALONE. ***
This script is BYTE-FOR-BYTE IDENTICAL to the currently-deployed
`ema_regime_crossover_swing_cnc_signal.py` (deployed copy:
`ema_regime_crossover_swing_cnc_signal_20260731001033.py`, live/paper since
2026-07-31, static `HIGH_BETA_15` 15-name universe ranked from a 59-name pool,
"Run 3": Sharpe(net) 2.76, PF(net) 1.67, WR(net) 34.9%, maxDD 18.1%, 169 trades)
on every dimension EXCEPT the traded universe and its group map. It runs as a
SEPARATE, PARALLEL strategy in Sandbox/paper mode with its own state file, its
own capital-allocation key, and its own trade log -- it does NOT touch, read,
or modify anything belonging to the deployed Run 3 strategy.

WHY THIS EXISTS: `equities-system/strategies/ema_regime_crossover.md`, section
"High-Beta Top-15 Re-Ranked from Full Nifty 100 Pool (2026-08-01)" -- re-running
the exact same causal, no-lookahead annualized-volatility ranking methodology
already used to build the deployed universe (`ema_regime_crossover_high_beta_rank.py`),
but sourcing candidates from the FULL Nifty 100 pool (99 names) instead of the
original 59-name pool, still taking only the top 15 by that ranking. Backtest
result (`ema_regime_crossover_swing_cnc_high_beta_nifty100_backtest.py`,
2021-07-01->2026-06-30, 147 trades): Sharpe(net) 3.72, PF(net) 2.26, WR(net)
37.4%, maxDD 10.6% -- beats the deployed Run 3 config on every metric. Karan
confirmed this candidate "seems the best" of the whole campaign and explicitly
chose to forward-test it in PARALLEL alongside the deployed strategy, not swap
the deployed universe outright. See the strategy doc for the full evidence
trail and the proposed forward-test evaluation gate before any decision to
swap the deployed universe to this candidate.

Universe change only: 9 of 15 names carry over from the deployed Run 3 list
(ADANIGREEN, ADANIENSOL, ADANIENT, ADANIPOWER, ETERNAL, LODHA, TMCV, VEDL,
ADANIPORTS); 6 are new (MAZDOCK, MOTHERSON, IRFC, UNIONBANK, HINDZINC,
RECLTD -- Nifty Next 50 names outside the original 59-pool's fixed top-10
subset); 6 are dropped from the deployed list (CGPOWER, MAXHEALTH, CANBK,
SHRIRAMFIN, TRENT, JIOFIN, now below the new pool's top-15 cutoff). GROUP_OF
extended for the 6 new names using the exact group assignments already decided
in `ema_regime_crossover_swing_cnc_nifty100_backtest.py`'s full-Nifty-100 group
map: HINDZINC -> VEDANTA_GROUP (paired with VEDL, already in this universe);
MAZDOCK, MOTHERSON, IRFC, UNIONBANK, RECLTD each deliberately left as their own
singleton group (no PSU-conglomerate or standalone grouping invented here, per
that script's own documented rationale).

Separate strategy from ALL existing siblings:
  ...
  - ema_regime_crossover_swing_cnc_signal.py          (Swing CNC Run 3, DEPLOYED 2026-07-31, UNTOUCHED by this file)
...
with ONLY its universe/group-map swapped per the 2026-08-01 re-ranking test above.
It ports Run 3's exact trading logic -- EXACTLY, parameter-for-parameter, from the
backtest that IS the deployed sibling's spec byte-for-byte on trading logic:
strategies/backtests/ema_regime_crossover_swing_cnc_concentration_v3_tightstop_backtest.py.
Do not change INITIAL_STOP_ATR_MULT / TRAIL_ARM_ATR_MULT / TRAIL_ATR_MULT / MAX_HOLD_DAYS
/ the concentration-cap logic without re-running that backtest first --
frozen-strategy-settings rule applies here same as both other siblings. The universe/
group-map ARE the intentional variable under test here, per the 2026-08-01 backtest above
-- do not change them further without a new backtest to justify it.
...
Universe: the 15-name high-beta subset re-ranked from the FULL Nifty 100 pool
(2026-08-01 test, see module header above) -- NOT the deployed Run 3 sibling's
59-pool-ranked HIGH_BETA_15, NOT the intraday/RRG siblings' 20/59-name universes
-- see HIGH_BETA_15_NIFTY100 below. ADANIENSOL has historically failed to fetch
from Dhan/OpenAlgo in the original 59-pool backtest; the 2026-08-01 re-ranking
run fetched all 99 Nifty 100 names successfully (no data gaps) -- this script
still logs a warning and continues without any name that fails to fetch on a
given pass rather than treating it as fatal, same defensive pattern as the
deployed sibling. This is a STATIC list, no monthly ADV re-ranking (unlike the
intraday sibling's refresh_universe() mechanism) -- the re-ranking test that
produced this list was a one-off exercise, not an ongoing monthly process, and
re-deriving it live here would be a new, unverified mechanism outside this
task's scope.
...
Capital: reads allocated_capital via capital_state.get_allocated_capital("ema_regime_crossover_swing_cnc_top15nifty100")
-- its OWN key, added to capital_allocation.json this same session (paper mode, Rs
2,50,000 starting -- SAME base as the deployed Run 3 sibling's allocation, deliberately,
for a fair like-for-like forward-test comparison) -- entirely separate from the deployed
Run 3 sibling's "ema_regime_crossover_swing_cnc" key, the intraday sibling's
"ema_regime_crossover" key, and the RRG swing sibling's "ema_regime_crossover_swing" key.
Live-mode closed trades would compound into this key's own allocated_capital via
capital_state.record_trade(); paper-mode trades never touch it -- identical mechanism to
every sibling. This strategy stays in Sandbox/paper mode only.

State persisted to state/ema_regime_crossover_swing_cnc_top15nifty100_state.json --
ENTIRELY SEPARATE from every sibling's own state file (including the deployed Run 3
sibling's ema_regime_crossover_swing_cnc_state.json), per this task's explicit requirement
that this script must not share state with, or interfere with, ANY already-running
strategy. Schema: positions (symbol -> entry_price, qty, initial_stop, trail_stop,
highest_close, atr_at_entry, days_held, armed, entry_time, exit_order_placed/
pending_reverse_exit retry flags -- same retry-safety pattern as every sibling),
group_open_count (group -> count, rebuilt from positions on load so it can never drift out
of sync with the actual open-position set), last_signal_date (gates the once-per-day
signal pass the same way every sibling gates its own daily/intraday passes).

DEPLOYED IN PARALLEL (2026-08-01) alongside the already-deployed Run 3 sibling -- NOT a
replacement for it. Registered in strategy_configs.json under its own timestamped key,
same schedule (09:15-15:25 IST, mon-fri) as the Run 3 sibling for a fair like-for-like
forward-test comparison. This is a genuine live/paper strategy in Sandbox mode, not a
research-only script -- see equities-system/strategies/ema_regime_crossover.md (2026-08-01
addition to the "EMA Swing CNC" area) for the proposed forward-test evaluation gate that
determines when there is enough evidence to decide whether to swap the deployed Run 3
universe to this candidate.
"""
...
# ---- Universe: high-beta 15-name subset, RE-RANKED FROM THE FULL NIFTY 100 POOL
# (2026-08-01 test) -- NOT the deployed Run 3 sibling's 59-pool-ranked HIGH_BETA_15.
# See ema_regime_crossover_swing_cnc_high_beta_nifty100_backtest.py for the ranking
# run and full rationale. ----
...
# Adani-group entries reused verbatim from the deployed Run 3 sibling. The 6
# new names (MAZDOCK, MOTHERSON, IRFC, UNIONBANK, HINDZINC, RECLTD) checked
# against ema_regime_crossover_swing_cnc_nifty100_backtest.py's already-built
# full-Nifty-100 group map: HINDZINC is the only one of the 6 with a documented
# multi-member group there (VEDANTA_GROUP, paired with VEDL, already in this
# universe) -- carried over below verbatim. MAZDOCK (defense PSU), IRFC/RECLTD
# (PSU financiers), UNIONBANK (PSU bank), and MOTHERSON (standalone) are each
# deliberately left as singleton groups per that script's own documented
# rationale (PSU names not grouped as one conglomerate) -- not invented here.
GROUP_OF = {
...
STRATEGY_KEY = "ema_regime_crossover_swing_cnc_top15nifty100"
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "state",
                          "ema_regime_crossover_swing_cnc_top15nifty100_state.json")
...
def log_closed_trade(sym, pos, exit_px, pnl, reason, now):
    ...
    from every sibling's logs, including the deployed Run 3 sibling's
    (strategies/logs/ema_regime_crossover_swing_cnc_trades.csv), via
    capital_state.py's per-strategy-key file naming (this key logs to
    strategies/logs/ema_regime_crossover_swing_cnc_top15nifty100_trades.csv).
    Live mode also compounds pnl into this key's OWN allocated_capital."""
